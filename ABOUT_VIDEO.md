...
Если отправка chunks не удается (сервер недоступен), система автоматически переходит на **клиентское сохранение** - вся запись хранится в памяти браузера и загружается при завершении звонка.
